...
$(document).ready(function () {
  ...
  $(".fav-btn-nav").on("click", function () {
    window.location.replace("/favorites");
  });

  // Redirect page if new search is initiated
  $("#desktop-search").keypress(function (event) {
    if (event.which == 13) {
      event.preventDefault();
      //redirect to search page after pressing enter in search bar
      if ($("#desktop-search").val() != null) {
        window.location.href = "/search/" + $("#desktop-search").val();
      }
    }
  });
  ...
  $("#mobile-search").keypress(function (event) {
    if (event.which == 13) {
      event.preventDefault();
      //redirect to search page after pressing enter in search bar
      if ($("#mobile-search").val() != null) {
        window.location.href = "/search/" + $("#mobile-search").val();
      }
    }
  });
});
